# 2022.05.14-Changed for main script for wavemlp model
...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 12, 'input_size': (3, 224, 224), 'pool_size': None,
...
class PATM(nn.Module):
    def __init__(self, dim, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,mode='fc'):
        ...
        # self.tfc_h = nn.Conv2d(2*dim, dim, (1,3), stride=1, padding=(0,3//2), groups=dim, bias=False)
        # self.tfc_w = nn.Conv2d(2*dim, dim, (3,1), stride=1, padding=(3//2,0), groups=dim, bias=False)

        if mode=='fc':
            self.theta_h_conv=nn.Sequential(
                nn.Conv2d(dim, dim, 1, 1, bias=True),nn.BatchNorm2d(dim),nn.ReLU()
                )
            ...
            self.theta_h_conv=nn.Sequential(
                nn.Conv2d(dim, dim, 3, stride=1, padding=1, groups=dim, bias=False),nn.BatchNorm2d(dim),nn.ReLU())
            self.theta_w_conv=nn.Sequential(
                nn.Conv2d(dim, dim, 3, stride=1, padding=1, groups=dim, bias=False),nn.BatchNorm2d(dim),nn.ReLU()
                )
    ...
    def forward(self, x):
        ...
        theta_h=self.theta_h_conv(x)   # Conv 1×1,BN,ReLU,相角/虚部
        theta_w=self.theta_w_conv(x)   # Conv 1×1,BN,ReLU,相角/虚部
        x_h=self.fc_h(x)               # x的高度,Conv 1×1,幅值/实部
        x_w=self.fc_w(x)               # x的宽度,Conv 1×1,幅值/实部
...
@register_model
def WaveMLP_Tor(pretrained=False, **kwargs):
    transitions = [True, True, True, True]
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
